...
// todo: callbacks below down to mainwindowproc
...
BOOL EnumMonitorProc(HMONITOR hMonitor, HDC hdcMonitor, LPRECT lprcMonitor, LPARAM lParam);
BOOL EnumChildGetText(HWND aWnd, LPARAM lParam);
LRESULT MainWindowProc(HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM lParam);
...
*/		// Otherwise, it's a remote computer whose registry the caller wants us to open:
	...
	static char *RegConvertRootKey(char *aBuf, size_t aBufSize, HKEY aRootKey)  // todo: linux version
	{
...
*/
		if (aBufSize)                        *aBuf = '\0'; // Make it be the empty string for anything else.
	...
	static int RegConvertValueType(char *aValueType)  // todo: linux version
	{
...
// todo: 2 callbacks below
LRESULT GuiWindowProc(HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM lParam);  // CALLBACK 
LRESULT TabWindowProc(HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM lParam);  // CALLBACK 
