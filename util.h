...
*/
// g++ -o util.o util.h
// todo: problems with atoi, trying to use atoll
#ifndef util_h
#define util_h
...
EXTERN_G;  //  in defines.h: extern global_struct *g
// For ITOA() and related functions' use of g->FormatIntAsHex

#define IS_SPACE_OR_TAB(c) (c == ' ' || c == '\t')
#define IS_SPACE_OR_TAB_OR_NBSP(c) (c == ' ' || c == '\t' || c == -96) 
// Use a negative to support signed chars.
...
// todo: char cast issues originate here i think ~naveen
...
// drop the benchmarks significantly.
{
	...
	for (bool convert_next_alpha_char_to_upper = true; *aStr; ++aStr)
	{
		if (IsCharAlpha(*aStr)) // Use this to better support chars from non-English languages.
		{
			...
			{
			  *aStr = (char)toupper(*aStr);  // not locale independent
				convert_next_alpha_char_to_upper = false;
			}
			else
			  *aStr = (char)tolower(*aStr);  // not local independent
		}
		else
...
//#define ATOI64(buf) _strtoi64(buf, NULL, 0) // formerly used _atoi64()
{
  ...
  // _atoi64() has superior performance, so use it when possible.
  return atoll(buf);
}

inline unsigned long long ATOU64(char *buf)
{
...
inline char *ITOA(int value, char *buf)
{
	if (g->FormatIntAsHex)
	{
		...
		*our_buf_temp++ = '0';
		*our_buf_temp++ = 'x';
		//		itoa(value < 0 ? -(int)value : value, our_buf_temp, 16);
		sprintf(our_buf_temp, "%x", value < 0 ? -(int)value : value);
		// Must not return the result of the above because it's our_buf_temp and we want buf.
		return buf;
	}
	else
	  //		return itoa(value, buf, 10);
	 sprintf(buf, "%d", value); // itoa not available on linux
	return buf;
}
...
inline char *ITOA64(__int64 value, char *buf)
{
	if (g->FormatIntAsHex)
	{
		...
		*our_buf_temp++ = 'x';
		...
		sprintf(buf, "%llx", value < 0 ? -(__int64)value : value); // itoa not available on linux

		// llx is actually unsigned hex 64 bit... todo: output signed hex... 
		// Must not return the result of the above because it's our_buf_temp and we want buf.
	 ...
	 sprintf(buf, "%lld", value); // itoa not available on linux
	return buf;	  
}
...
inline char *UTOA(unsigned long value, char *buf)
{
	if (g->FormatIntAsHex)
	{
	  ...
	  sprintf(buf, "%lu", value); // itoa not available on linux
	return buf;	  
...
uint64_t GetFileSize64(HANDLE aFileHandle);
...
// TODO: need linux version of GetVirtualDesktopRect(RECT &aRect);
...
// todo: make fontenumproc a function pointer (callback), translate to x11
// int FontEnumProc(ENUMLOGFONTEX *lpelfe, NEWTEXTMETRICEX *lpntme, DWORD FontType, LPARAM lParam);
